...
def validate_skill_name(name: str, skill_dir: Optional[Path] = None) -> List[str]:
    ...
    Args:
        name: skill 名称
        skill_dir: skill 目录路径（可选，用于验证目录名匹配）
...
def validate_skill_directory(skill_dir: Path) -> List[str]:
    ...
    return validate_frontmatter(metadata, skill_dir)
...
def validate_skill_name_simple(name: str) -> bool:
    """
    简单验证 skill 名称（向后兼容）
